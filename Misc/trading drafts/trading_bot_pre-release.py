...
def get_active_symbols():
    ...
    for symbol in ALL_SYMBOLS:
        try:
            prices = get_recent_close(symbol, minutes=2)[-2:]
            if len(prices) == 2 and prices[1] > prices[0]:
                growth = (prices[1] - prices[0]) / prices[0]
                trends.append((symbol, growth))
        except:
            continue
    sorted_trends = sorted(trends, key=lambda x: x[1], reverse=True)
    top_symbols = [s for s, _ in sorted_trends[:20]]
    print(f"[INFO] Top 20 trending symbols selected: {top_symbols}")
    return top_symbols
        ...
        for symbol in SYMBOLS:
            try:
                ...
                if len(positions) < MAX_OPEN_POSITIONS and symbol not in positions and should_enter(prices):
                    ...
                    if current_price >= entry * 1.002 or current_price <= entry * 0.997:
